...
def create_json():
    ...
    for dictionary in req_data:
        list_res.append({
            ...
            "score": coScore.coScore(dictionary["a.name"], dictionary["b.name"]) + pmc_api.get_number_of_results(dictionary["a.name"], dictionary["b.name"])
        })
    dict_res["items"] = list_res
    print(dict_res)
    with open((Path(__file__).parent / "./output_scoring.json").resolve(), "w") as fout:
